...
export default function CategoryChips({ categories, variant = 'sticky' }: Props) {
  if (!categories || categories.length === 0) return null;
  const wrapperClasses =
    variant === 'sticky'
      ? 'md:hidden sticky top-14 z-40 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 border-b border-gray-100'
      : 'md:hidden w-full';
