...
// Hooks comentados temporalmente para evitar bucles infinitos
...
// import { useServicePreloader } from '@/hooks/useServicePreloader';
  ...
  interface ServicesContextType {
    ...
    // isPreloading: boolean; // Temporalmente comentado
  ...
  export const ServicesProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // 🚀 Integrar sistema de cache optimizado - COMENTADO TEMPORALMENTE
    // const {
    ...
    // 📊 Integrar sistema de analytics y precarga - COMENTADO TEMPORALMENTE
    // const {
    //   trackServiceVisit,
    //   getAnalyticsStats
    // } = useServiceAnalytics();
    ...
    // Funciones mock temporales
  const trackServiceVisit = () => {};
  ...
  const forcePreload = async () => {};
    ...
    const loadLocalServices = () => {
      try {
        console.log('🔍 DEBUG: Iniciando carga de servicios locales');
        console.log('🔍 DEBUG: localServicesData disponible:', !!localServicesData);
        console.log('🔍 DEBUG: localServicesData length:', localServicesData?.length);
        
        if (!localServicesData || localServicesData.length === 0) {
          console.log('⚠️ No hay datos locales, usando mockServices');
        ...
        console.log('🔍 DEBUG: Primeros 3 servicios:', activeServices.slice(0, 3).map(s => ({ id: s.id, name: s.name })));
      } catch (error) {
        console.error('❌ Error cargando datos locales:', error);
        console.log('🔄 Fallback a mockServices');
        setAllServices(mockServices);
      }
    };
    ...
    const loadFeaturedServices = useCallback(async (forceRefresh = false) => {
      try {
        ...
        // 🚀 OPTIMIZACIÓN: Verificar cache localStorage primero (temporalmente comentado)
        // if (!forceRefresh) {
        ...
        if (process.env.NEXT_PUBLIC_DISABLE_FIREBASE === 'true') {
          console.log('❌ Firebase deshabilitado por configuración, usando servicios destacados mock');
        ...
        // setFeaturedServicesCache(activeFeatured); // Temporalmente comentado
    ...
    }, [db, featuredCache, cacheExpiry]); // Dependencias de funciones de cache comentadas temporalmente
    ...
    }, [db, singleServiceCache, cacheExpiry, setLocalSingleServiceCache]); // getSingleServiceFromCache comentado temporalmente
    ...
    const loadServicesPaginated = useCallback(async (page: number, pageSize: number, forceRefresh: boolean = false) => {
      try {
        ...
        // Verificar si Firebase está deshabilitado
        if (process.env.NEXT_PUBLIC_DISABLE_FIREBASE === 'true') {
          console.log('❌ Firebase deshabilitado por configuración, usando datos mock para paginación');
        ...
        // 🚀 OPTIMIZACIÓN MEJORADA: Verificar cache localStorage con timestamp (temporalmente comentado)
        // if (!forceRefresh) {
        ...
        // 💾 Guardar en cache localStorage solo la primera página (temporalmente comentado)
    ...
    }, [db, lastDocumentSnapshot, paginatedServices]); // getAllServicesFromCache, setAllServicesCache comentados temporalmente
    ...
    const loadServicesFromFirestore = async (forceRefresh = false) => {
      try {
        ...
        // Verificar si Firebase está deshabilitado
        if (process.env.NEXT_PUBLIC_DISABLE_FIREBASE === 'true') {
          console.log('🚫 Firebase disabled, using local services');
          loadLocalServices();
          setUsingMockData(true);
          return;
        }
        ...
        if (db) {
          try {
            ...
            const queryPromise = limitedGetDocs(servicesQuery);
            const timeoutPromise = new Promise((_, reject) => 
              setTimeout(() => reject(new Error('Firestore query timeout')), 10000)
            );
            
            const servicesSnapshot = await Promise.race([queryPromise, timeoutPromise]) as any;
            
            const servicesData = servicesSnapshot.docs.map((doc: any) => {
              const data = doc.data();
              return {
                id: doc.id,
                ...data,
            ...
            // 💾 Guardar en cache localStorage (temporalmente comentado)
        ...
        if (errorMessage.includes('timeout') || errorMessage.includes('network') || errorMessage.includes('UNAVAILABLE')) {
          setError('Problema de conectividad. Usando datos locales.');
          console.log('🌐 Network/timeout error, using fallback services');
        } else if (error?.code === 'resource-exhausted' || 
                   errorMessage.includes('Quota exceeded') ||
                   errorMessage.includes('429')) {
          console.warn('Cuota de Firebase excedida, usando datos locales como fallback');
          setError('Límite de consultas alcanzado. Usando datos locales.');
          toast('Conectando con datos locales debido a límites temporales del servidor', { icon: 'ℹ️' });
          scheduleRetry();
        } else {
          setError(`Error cargando servicios: ${errorMessage}`);
          console.log('🔄 General error, falling back to mock services');
        }
    ...
    const initializeServices = useCallback(async () => {
      ...
      setLoading(true);
      try {
        // Intentar cargar desde cache primero (temporalmente comentado)
        // const cachedServices = getAllServicesFromCache();
        // if (cachedServices && cachedServices.length > 0) {
        //   setAllServices(cachedServices);
        //   setUsingMockData(false);
        ...
        console.log('🔥 Inicializando con datos reales desde Firebase');
    ...
    }, [allServices.length]); // getAllServicesFromCache comentado temporalmente
    ...
    const refreshServices = useCallback(async (forceRefresh = false) => {
      console.log('🔄 DEBUG: refreshServices() ejecutándose');
      console.log('🔄 DEBUG: forceRefresh =', forceRefresh);
      console.log('🔄 DEBUG: NEXT_PUBLIC_DISABLE_FIREBASE =', process.env.NEXT_PUBLIC_DISABLE_FIREBASE);
      
      if (process.env.NEXT_PUBLIC_DISABLE_FIREBASE === 'true') {
        console.log('🚫 Firebase deshabilitado, cargando servicios locales');
        loadLocalServices();
        setUsingMockData(true);
        setLoading(false);
      } else {
        console.log('🔥 Firebase habilitado, cargando desde Firestore');
    ...
    // Hook para detectar cuando Firebase está listo
    const { isReady: firebaseReady, isClient } = useFirebaseReady();
    
    // Debug del hook
    console.log('🔧 ServicesContext - Hook values:', { firebaseReady, isClient });

    // Inicializar servicios cuando Firebase esté listo
    useEffect(() => {
      console.log('🚀 ServicesProvider - Firebase ready effect');
      ...
      console.log('🔍 DEBUG: usingMockData =', usingMockData);
      
      if (firebaseReady && allServices.length === 0 && !loading) {
        console.log('🌐 Firebase está listo, iniciando carga de servicios...');
        refreshServices(true);
      } else if (firebaseReady && allServices.length > 0) {
        console.log(`✅ Firebase listo y servicios ya cargados: ${allServices.length} servicios disponibles`);
      } else if (!firebaseReady && isClient) {
        console.log('⏳ Esperando a que Firebase esté listo...');
      } else if (!isClient) {
        console.log('🖥️ Ejecutándose en el servidor, saltando carga de servicios');
      }
    }, [firebaseReady, allServices.length, loading, refreshServices]);
    ...
    const value = useMemo(() => ({
      ...
      preloadPopularServices,
      forcePreload,
      // isPreloading, // Temporalmente comentado
      // Función para cargar servicios desde Firestore
      loadServicesFromFirestore,
      // Control del limitador de lecturas Firebase
      ...
      forcePreload, loadServicesFromFirestore,
      getFirebaseReadsLimiter
    ]);
